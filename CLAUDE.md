...
- ✅ **Camera counting** (`Features/CameraCount/`) — feature 1. `CellCounter` tier ladder;
  `ClassicalCellCounter` (Otsu + connected components) is the floor. Core ML tiers plug in via
  `CoreMLCounterSpec.bundled` (currently empty). Uploads image + 16-bit instance mask + CSVs.
...
- Classical counter merges touching cells → add distance-transform **watershed** splitting.
- Google Drive provider + OAuth (feature 4 foundation).
- Convert a Cellpose / small-SAM model → Core ML for the higher counter tiers.
- Full-resolution masks (currently capped at `SegmentationConfig.maxDimension` = 2048).
